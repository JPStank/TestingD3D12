...
static Application* gSingleton = nullptr;
static WindowMap gWindows;
static WindowNameMap gWindowByName;
	...
	, mTearingSupported(false)
{
	...
	wndClass.hIcon = LoadIcon(mHinstance, NULL);
	wndClass.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wndClass.lpszMenuName = nullptr;
	wndClass.lpszClassName = WINDOW_CLASS_NAME;
	wndClass.hIconSm = LoadIcon(mHinstance, NULL);
...
void Application::Create(HINSTANCE hInst)
{
	if (!gSingleton)
	{
		gSingleton = new Application(hInst);
	}
}
...
void Application::Destroy()
{
	if (gSingleton)
	{
		assert(gWindows.empty() && gWindowByName.empty() &&
...
std::shared_ptr<Window> Application::CreateRenderWindow(const std::wstring& windowName, int clientWidth, int clientHeight, bool vSync)
{
	...
	WindowNameMap::iterator windowIter = gWindowByName.find(windowName);
	if (windowIter != gWindowByName.end())
	{
	...
	gWindows.insert(WindowMap::value_type(hWnd, pWindow));
	gWindowByName.insert(WindowNameMap::value_type(windowName, pWindow));
...
std::shared_ptr<Window> Application::GetWindowByName(const std::wstring& windowName)
{
	...
	WindowNameMap::iterator iter = gWindowByName.find(windowName);
	if (iter != gWindowByName.end())
	{
		window = iter->second;
	}
...
{
	WindowMap::iterator windowIter = gWindows.find(hWnd);
	if (windowIter != gWindows.end())
	{
		WindowPtr pWindow = windowIter->second;
		gWindowByName.erase(pWindow->GetWindowName());
		gWindows.erase(windowIter);
	}
}
